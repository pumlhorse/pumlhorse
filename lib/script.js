...
function getScript(obj) {
    ...
    function useModule(descriptor) {
        ...
        if (helpers.isObject(descriptor)) {
        ...
        var m = modules.modules[moduleName]
        if (!m) throw new Error("Module '" + moduleName + "' does not exist")
        
        if (moduleNamespace.length > 0) {
            var ns = {};
            helpers.assignObjectByString(ns, moduleNamespace, m)
            m = ns;
        }
    ...
    function runStep(step, scope) {
        ...
        function runFunction(functionName, funcParameters) {
            ...
            var evalParameters = {};
            if (funcParameters && funcParameters.constructor === Array) evalParameters = funcParameters.map(compileParameter)
            else if (typeof funcParameters === "object") evalParameters = _.mapObject(funcParameters, compileParameter)
            ...
            return Promise.mapSeries(functionParameterNames, getParameter)
                .then(function (params) {
                    ...
                    if (runFunc.__passAsObject) stringParams = stringify([evalParameters])
                    else if (helpers.isValueType(evalParameters)) stringParams = evalParameters
                    else if (helpers.isString(evalParameters)) stringParams = evalParameters
                    else if (helpers.isArray(evalParameters)) stringParams = stringify(evalParameters)
                    else stringParams = stringify(params)
                    
                    var result = Expression.compile(functionName + '(' + stringParams + ')')(_scope)
            ...
            function stringify(arr) {
                return arr.map(function (x) {
                    ...
                    if (x.constructor === String) return x
    ...
    function createFunction(val) {
        ...
        if (val.constructor === String) return new Function(val)
