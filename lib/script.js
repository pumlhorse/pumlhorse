...
function getScript(obj) {
    ...
    function useModule(descriptor) {
        ...
        if (moduleNamespace.length > 0) {
            helpers.assignObjectByString(_functions, moduleNamespace, m)
        }
        else {
    ...
    function runStep(step, scope) {
        ...
        function runFunction(functionName, funcParameters) {
            ...
            if (funcParameters && helpers.isArray(funcParameters)) evalParameters = funcParameters.map(compileParameter)
            else if (funcParameters && helpers.isObject(funcParameters)) evalParameters = _.mapObject(funcParameters, compileParameter)
            else if (funcParameters) evalParameters = compileParameter(funcParameters)
            ...
            return Promise.mapSeries(functionParameterNames, getParameter)
                .then(function (params) {
                    ...
                    else if (helpers.isString(evalParameters)) passedParams = [evalParameters]
                    else if (helpers.isArray(evalParameters) && helpers.isArray(funcParameters)) passedParams = evalParameters
                    else if (helpers.isString(funcParameters) && helpers.isObject(evalParameters)) passedParams = [evalParameters]
