...
function runProfile(profile, sessionOutput) {
    ...
    function buildContext() {
        ...
        if (!contexts || contexts.length == 0) return Promise.resolve({});
    ...
    function runScript(s) {
        ...
        function loadModules(scriptObj) {
            ...
            if (!scriptObj.script.modules) return Promise.resolve({});
            
            var scriptDir = path.dirname(scriptObj.filePath)
            return Promise.map(scriptObj.script.modules, function (mod) {
                return resolveModule(mod, scriptDir)
            })
        }
        
        function resolveModule(mod, directory) {
            if (helpers.isString(mod)) return resolveNodeModule(directory, mod);
            
            if (helpers.isObject(mod)) {
                
                var keys = Object.keys(mod)
                
                if (keys.length > 1) throw new Error(i18n.invalid_module_format_separate_items)
                
                var pumlhorseName = keys[0]
                var descriptor = mod[pumlhorseName]
                
                if (path.isAbsolute(descriptor)) return resolveFile(pumlhorseName, descriptor);
                if (descriptor[0] == '.') return resolveFile(pumlhorseName, path.resolve(directory, descriptor))
                
                return resolveNodeModule(directory, descriptor);            
            }
        
            function resolveFile(moduleName, path) {
                try {
                    return require(path)
                }
                catch (e) {
                    throw new Error("Unable to resolve module '" + moduleName + "'. " + (e.message ? e.message : e))
                }
            }
            
            function resolveNodeModule(directory, moduleName) {
                try {
                    return require(moduleName)
                }
                catch (e) {
                    return resolveFromScriptDirectory(moduleName, directory)
                    
                }
            
                function resolveFromScriptDirectory(name, directory) {
                    var append = "puml_modules/" + name
                    var relative = path.resolve(directory, append)                
                    var mod;
                    while (!mod) {
                        try {
                            ...
                            if (e.code !== "MODULE_NOT_FOUND") {
                                throw new Error("Error resolving module '" + name + "'. " + (e.message ? e.message : e))
                            }
                            var parentDir = path.resolve(relative, "../../../", append)
                            
                            if (parentDir === relative) break;
                            relative = parentDir;
                        }
                    }
                    ...
                    throw new Error("Unable to resolve module '" + moduleName + "'. Make sure you have installed it locally")
                }
            }
        }
    }
